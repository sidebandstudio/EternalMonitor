    ...
    if ui.is_rect_visible(rect) {
        ...
        let (fill, stroke, text) = match tone {
            ...
            Tone::Ghost => (
                ...
                Stroke::NONE,
                if hovered { TEXT } else { TEXT_MUTED },
            ),
        };
...
pub fn segmented<T: PartialEq + Copy>(ui: &mut Ui, value: &mut T, options: &[(T, &str)]) -> bool {
    ...
    for (i, ((option, label), galley)) in options.iter().zip(galleys).enumerate() {
        ...
        let response = ui.interact(rect, base_id.with(i), Sense::click());
        ...
        if selected {
            ...
            ui.painter().rect_filled(rect, 7.0, SURFACE);
        }
...
pub fn slider(
    ...
    if ui.is_rect_visible(rect) {
        ...
        let radius = if active { 8.5 } else { 7.5 };
...
/// Coloured notice with an icon, title, body and optional actions.
pub fn banner(
    ui: &mut Ui,
    color: Color32,
    icon: Icon,
